<template>
  <div relative flex flex-col min-h-0 px-8 pt-5 pb-12>
    ...
    <div v-else w-full c-gray-300 text-center text-sm p-5>暂无好友，点击下方按钮添加吧</div>
    <div text-sm color-gray mb-2>群聊</div>
    <div v-if="groupList.length > 0" grid grid-cols-3 gap-x-5>
      <chat-contact-item v-for="item in groupList" :key="item.contactId" v-bind="{ mode: 'chat', ...item }" />
    </div>
    <div v-else w-full c-gray-300 text-center text-sm p-2>暂未加入任何群</div>
